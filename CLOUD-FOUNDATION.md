...
- **Open — open-source line (decide at the billing/packaging phase):** DriftGrid is MIT today (`jsbzy/driftgrid`). The architecture maps cleanly onto open-core: the **local-first product stays open source** (editor, drift/branch/rounds, agents, the SQLite backend, the `lib/storage` dispatch — the free "runs on your hardware" half that drives adoption), while the **hosted cloud layer is the commercial part** (multi-tenant Postgres + RLS, hosted web MCP, teams, billing, branding removal). The genuine fork: do we *also* open-source the Postgres/migration/cloud backend so people can self-host their own cloud (more goodwill + contributors, but easier to clone the hosted business), or keep that layer source-available/closed (cleaner moat)? Leaning: MIT core + closed/source-available hosted layer. Nothing in the code forces either way.
- **Open — Postgres write atomicity (blocks Phase 4, see PHASE-4-DESIGN.md):** PostgREST/`supabase-js` has no client-side transactions, so the cloud decompose-write needs either a plpgsql RPC (manifest jsonb → server-side transactional upsert+prune) or a direct `pg` connection. Pick before building the cloud backend.
